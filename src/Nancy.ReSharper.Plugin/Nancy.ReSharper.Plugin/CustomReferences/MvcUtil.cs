...
namespace Nancy.ReSharper.Plugin.CustomReferences
{
    public static class MvcUtil   
    {
        ...
        public static OneToListMap<string, IClass> GetAvailableControllers([NotNull] IPsiModule module, [CanBeNull] ICollection<string> areas = null, bool includingIntermediateControllers = false, ITypeElement baseClass = null)
        {
            ...
            return GetAvailableControllers(module, searchDomain, includingIntermediateControllers, baseClass);
        }
        ...
        /// <returns></returns>
        public static OneToListMap<string, IClass> GetAvailableControllers([NotNull] IPsiModule module, [NotNull] ISearchDomain searchDomain, bool includingIntermediateControllers = false, ITypeElement baseClass = null)
        {
            ...
            if (!includingIntermediateControllers)
                classes = classes
                  .Where(@class => !@class.IsAbstract && @class.ShortName.EndsWith(ControllerClassSuffix, StringComparison.OrdinalIgnoreCase));
        ...
        private static readonly IDictionary<string, MvcKind> ourMvcKinds = new Dictionary<string, MvcKind>
    {
          ...
          new Dictionary<MvcKind, Func<IAttributeInstance, string>>
      {
        ...
        {
            return controllerName.TrimFromEnd(ControllerClassSuffix);
        }
        ...
        public static ICollection<JetTuple<string, string, JetBrains.ReSharper.Feature.Services.Asp.CustomReferences.MvcUtil.DeterminationKind, ICollection<IClass>>> GetControllers([CanBeNull] IArgumentsOwner argumentsOwner)
        {
        ...
        private static ICollection<JetTuple<string, string, JetBrains.ReSharper.Feature.Services.Asp.CustomReferences.MvcUtil.DeterminationKind, ICollection<IClass>>> GetControllersNonCached([NotNull] IArgumentsOwner argumentsOwner)
        {
            ...
            if (controllerNames.IsEmpty())
            {
                ...
                if (projectFile.LanguageType.Is<HtmlProjectFileType>())
                {
                    ...
                    (
                      ...
                      let viewsPath = new FileSystemPath(string.Format(locationFormat, null, ControllerClassSuffix /* just fake */, area)).Directory.Directory
                ...
                if (names.IsEmpty())
                {
                    ...
                    if (declaredElement.IsAbstract)
                        // with inheritors
                        return GetAvailableControllers(psiModule, baseClass: declaredElement)
            ...
            (
              from tuple in names
              let availableControllers = GetAvailableControllers(psiModule, new[] { tuple.A }, tuple.D)
        ...
        private static IList<string> ProcessArgumentsExpression(IArgumentsOwner argumentsOwner, MvcKind kind)
        {
            ...
            return new List<string>(
              ...
              literalExpression =>
              {
                  ...
                  GetNancyLiteral(literalExpression, out invocationInfo, out initializerName);
          ...
          new Dictionary<MvcKind, MvcResolveErrorType>
      {
        ...
        [CanBeNull]
        public static IExpression GetNancyLiteral<TExpression>([NotNull] IExpression literal, [CanBeNull] out TExpression expression, [CanBeNull] out string anonymousPropertyName)
          where TExpression : class, IInvocationInfo
        {
            ...
            var analyser = LanguageManager.Instance.TryGetService<IAnonymousObjectsAnalyser>(literal.Language);
            if (analyser != null)
            {
                var anonymousExpression = literal.GetContainingNode<ITreeNode>(element => analyser.IsCreationExpression(element));
                if (anonymousExpression != null)
                {
                    var memberInitializer = analyser.GetMemberInitializers(anonymousExpression).FirstOrDefault(pair => ReferenceEquals(pair.Second, literal));
